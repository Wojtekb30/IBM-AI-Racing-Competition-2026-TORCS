...
BRAKE_THRESHOLD = 0.12  # Angle threshold for braking. Lower values brake earlier.
GEAR_SPEEDS = [0, 20, 40, 80, 100, 140]  # Speed thresholds for gear shifting.
...
def drive_modular(c):
    ...
    front_dist = max(front_dist, 1) #do not allow to sqrt negative values

    safe_speed = math.sqrt(front_dist) * 16.5
    target_speed = min(TARGET_SPEED, safe_speed)

    if S['speedX'] > target_speed:
        brake = min(1.0, (S['speedX'] - target_speed) / 20.0)
        accel = 0.0
    else:
        brake = 0.0
        accel = calculate_throttle(S, R)
